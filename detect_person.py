...
def run(
    ...
    Host = '192.168.45.40'  # 통신할 대상의 IP 주소
...
def Condition(client_sock, xywh, xyxy, gn, cls, count):
    ...
    try:
        if (((time_diff > 10) and (label == 0) and (area > 500)) or (
                (time_diff > 5) and (label == 0) and (area > 500) and (count >= 2))):
            json_object = {"id": 1, "label": int(cls), "timee": now_time}
